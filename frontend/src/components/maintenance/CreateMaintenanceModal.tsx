...
export const CreateMaintenanceModal: React.FC<CreateMaintenanceModalProps> = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Failed to submit maintenance ticket');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 dark:bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white dark:bg-[#111] border border-slate-200 dark:border-white/10 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col">
        ...
        <div className="p-6">
          ...
          <form id="create-maintenance-form" onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 dark:text-white/80">Issue Description *</label>
              <textarea 
                ...
                className="w-full bg-slate-50 dark:bg-[#1a1a1a] border border-slate-200 dark:border-white/10 rounded-xl py-3 px-4 text-slate-800 dark:text-white focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all custom-scrollbar"
              />
            </div>

            <div className="space-y-2">
              <label className="text-sm font-bold text-slate-700 dark:text-white/80">Priority Level *</label>
              <select 
                required
        ...
        <div className="p-6 border-t border-slate-100 dark:border-white/10 bg-slate-50/50 dark:bg-white/5 flex justify-end gap-3">
          ...
          <button type="submit" form="create-maintenance-form" disabled={loading} className="flex items-center gap-2 bg-gradient-brand text-white px-6 py-2.5 rounded-xl font-bold shadow-md hover:opacity-90 transition-opacity disabled:opacity-50">
